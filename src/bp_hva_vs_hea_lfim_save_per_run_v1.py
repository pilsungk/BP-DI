...
#
# Compare HEA vs HVA on longitudinal-field Ising model (LFIM) using
# destructive-interference diagnostics.
#
# Default run:
...
# - HVA:
#     each layer = exp(-i beta_x sum X_i)
#                then exp(-i gamma_zz sum Z_i Z_{i+1})
...
def make_timestamped_outdir(root):
    ...
    outdir = os.path.join(root, "bp_hva_vs_hea_lfim_" + ts)
...
def build_h_lfim_obc_matrix(n, hx=1.0, hz=0.5):
...
def pauli_terms_for_h_lfim_obc(n, hx=1.0, hz=0.5):
    terms = []
    for i in range(n - 1):
        terms.append(("zz", i, i + 1, -1.0))
    for i in range(n):
        terms.append(("x", i, None, hx))
    for i in range(n):
        terms.append(("z", i, None, hz))
    return terms
...
def build_h_term_matrices(n, hx=1.0, hz=0.5):
    mats = []
    for term in pauli_terms_for_h_lfim_obc(n, hx, hz):
...
def final_state_sv(params, n, depth, variant):
    ...
    if variant == "hva":
        for _layer in range(depth):
            ...
            for q in range(n):
                psi = apply_gate_sv(psi, rx_mat(2.0 * beta_x), q, n)

            for q in range(n - 1):
                psi = get_rzz(2.0 * gamma_zz, q, q + 1, n) @ psi

            for q in range(n):
                psi = apply_gate_sv(psi, rz_mat(2.0 * beta_z), q, n)
...
def summarize_layer_locality(values_k, n, depth, variant):
    vals = []
    per_layer = 2 * n if variant == "hea" else 3
...
def run_single(n, depth, variant, seed, H_mat, term_mats):
    rng = np.random.RandomState(seed)
    npar = n_params_for_variant(n, depth, variant)
    params = rng.uniform(RNG_INIT_LOW, RNG_INIT_HIGH, size=npar).astype(np.float64)

    grad_full = grad_sv(params, n, depth, H_mat, variant)
    grad_terms = grad_termwise_sv(params, n, depth, term_mats, variant)
    ...
    var_bridge_ratio = float(var_bridge_actual / var_bridge_approx) if var_bridge_approx > 1e-15 else 0.0
...
def build_summary(all_runs):
    ...
    for key in sorted(grouped.keys()):
        ...
        group_var_bridge_ratio = float(group_var_bridge_actual / group_var_bridge_approx) if group_var_bridge_approx > 1e-15 else 0.0
        ...
        for run in lst:
            run_id = "n{0}_d{1}_{2}_seed{3:02d}".format(n, depth, variant, run["seed"])
...
def save_raw_run(outdir, run):
    key = "n{0}_d{1}_{2}_seed{3:02d}".format(
        run["n_qubits"], run["depth"], run["circuit_variant"], run["seed"]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare HEA vs HVA on LFIM using destructive-interference diagnostics."
    )
    ...
    parser.add_argument("--hx", type=float, default=HX_FIELD)
    parser.add_argument("--hz", type=float, default=HZ_FIELD)
    ...
    cfg = {
        ...
        "hamiltonian": "LFIM_OBC",
        "hx_field": hx,
        "hz_field": hz,
        "variants": ["hea", "hva"],
        "hea_definition": "layerwise RY-RZ on each qubit plus ring entangler",
        "hva_definition": "RX field block plus nearest-neighbor RZZ block plus on-site RZ block",
    ...
    for n, depth, variant, seed in product(n_qubits_list, depths, variants, range(seeds)):
        if n not in h_cache:
            h_cache[n] = build_h_lfim_obc_matrix(n, hx=hx, hz=hz)
            term_cache[n] = build_h_term_matrices(n, hx=hx, hz=hz)
